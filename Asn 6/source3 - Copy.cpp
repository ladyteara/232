...
Description: Assignment from hell.
			 Still have no idea how to throw an exception.
*/
...
{		//Insert element into the list before itr
	...
	n->next = itr.nodePosition;			//link new to current node
	if
...
/* It's a best practice to have a separate exception class for each real class. */
class ListException: public SLLException
{					//generic DEque exception
public:
	ListException(const string& err): SLLException(err) {};		//constructor
};

class ListEmpty: public ListException
{						//invalid operation on empty DEque
public:
	ListEmpty(const string& err): ListException(err) {}			//constructor
};
